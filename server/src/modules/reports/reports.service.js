...
async function getRoi() {
  ...
  const results = vehicles.map((v) => {
    ...
    // Revenue estimated at $100/completed trip since no revenue field exists in the schema.
    // In production, this would come from a billing/invoice table linked to trips.
...
async function exportCsv() {
  ...
  const rows = trips
    .map((t) => {
      const vehicleReg = t.vehicle?.registrationNumber || "";
      const driverName = t.driver?.user?.name || "";
      const distance = t.actualDistance !== null && t.actualDistance !== undefined ? t.actualDistance : t.plannedDistance;
      return `${t.id},"${t.source}","${t.destination}",${t.status},"${vehicleReg}",${t.cargoWeight},${distance},"${driverName}",${t.createdAt.toISOString()}`;
    })
    .join("\n");
